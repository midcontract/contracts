...
test-contract-fork :; source .env && forge test --match-contract ExecuteEscrowHourlyEndToEndTest --rpc-url ${FORK_URL} -vvv
...
deploy-hourly :; source .env && forge script script/deploy/04_DeployEscrowHourly.s.sol:DeployEscrowHourlyScript --rpc-url ${DEPLOY_URL} --private-key ${DEPLOYER_PRIVATE_KEY} --broadcast --verify --etherscan-api-key ${SCAN_API_KEY} --gas-price ${GAS_PRICE} --gas-limit ${GAS_LIMIT} -vvvv
deploy-factory :; source .env && forge script script/deploy/05_DeployEscrowFactory.s.sol:DeployEscrowFactoryScript --rpc-url ${DEPLOY_URL} --private-key ${DEPLOYER_PRIVATE_KEY} --broadcast --verify --etherscan-api-key ${SCAN_API_KEY} -vvvv
deploy-feemanager :; source .env && forge script script/deploy/06_DeployEscrowFeeManager.s.sol:DeployEscrowFeeManagerScript --rpc-url ${DEPLOY_URL} --private-key ${DEPLOYER_PRIVATE_KEY} --broadcast --verify --etherscan-api-key ${SCAN_API_KEY} -vvvv
